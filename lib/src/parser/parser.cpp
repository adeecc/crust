...
std::unique_ptr<FnDecl> Parser::parseFnDecl() {
    ...
    while (mCurrentToken != Lexer::Token::EOF and
...
std::unique_ptr<ArgList> Parser::parseArgList() {
    ...
    while (mCurrentToken != Lexer::Token::EOF and
           (mCurrentToken < Lexer::Token::KW_INT_32 and mCurrentToken > Lexer::KW_VOID) and
...
std::unique_ptr<ExpressionRHS> Parser::parseExpressionRHS() {
    ...
    if (mCurrentToken >= Lexer::Token::OP_PLUS and mCurrentTokne <= Lexer::Token::OP_LT) {
...
std::unique_ptr<Call> Parser::parseCall() {
    ...
    while (mCurrentToken != Lexer::Token::EOF and
           ...
           mCurrentToken != Lexer::Token::COMMA and
           mCurrentToken != Lexer::Token::RPAREN and
           mCurrentToken != Lexer::Token::SEMI_COLON mCurrentToken != Lexer::Token::LBRACE) {
        ErrorLogger::printErrorAtLocation(ErrorLogger::ErrorType::VAR_MISSING_IDENTIFIER, mLexer.GetCurrentLocation());
        mCurrentToken = mLexer.getNextToken();
    }
...
std::unique_ptr<ElifBlock> Parser::parseElifBlock() {
    ...
    if (mCurrentToken == Lexer::Token::KW_ELIF) {
        std::unique_pair<Token> kw_elif = parseToken(Lexer::Token::KW_ELIF);
        std::unique_pair<Expression> exp = parseExpression();
        std::unique_pair<Token> lbrace = parseToken(Lexer::Token::LBRACE);
        std::unique_pair<StmtList> stmtList = parseStmtList();
        std::unique_pair<Token> rbrace = parseToken(Lexer::Token::RBRACE);
...
std::unique_ptr<ElseBlock> Parser::parseElseBlock() {
    ...
    if (mCurrentToken == Lexer::Token::KW_ELSE) {
        std::unique_pair<Token> kw_else = parseToken(Lexer::Token::KW_ELSE);
        std::unique_pair<Token> lbrace = parseToken(Lexer::Token::LBRACE);
        std::unique_pair<StmtList> stmtList = parseStmtList();
        std::unique_pair<Token> rbrace = parseToken(Lexer::Token::RBRACE);
