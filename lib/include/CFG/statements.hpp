...
class ReturnVar : public CFGNode {
   public:
    ReturnVar(std::unique_ptr<CFGNode>&& expression) : CFGNode{NodeKind::RETURN_VAR} {
        addChildNode(std::move(expression));
    }

    ReturnVar() : CFGNode{NodeKind::RETURN_VAR} {}
};

class WhileLoop : public CFGNode {
   public:
    WhileLoop() : CFGNode(NodeKind::ERROR) {}
    ...
    WhileLoop(std::unique_ptr<CFGNode>&& kw_while,
              ...
              std::unique_ptr<CFGNode>&& rbrace) : CFGNode(NodeKind::WHILE_LOOP) {
...
class LoopStep : public CFGNode {
   public:
    LoopStep(std::unique_ptr<CFGNode>&& range, std::unique_ptr<CFGNode>&& int_literal) : CFGNode(NodeKind::LOOP_STEP) {
        addChildNode(std::move(range));
        addChildNode(std::move(int_literal));
    }

    LoopStep() : CFGNode(NodeKind::LOOP_STEP) {}
};

class LoopRange : public CFGNode {
   public:
    LoopRange() : CFGNode(NodeKind::ERROR) {}

    LoopRange(std::unique_ptr<CFGNode>&& int_literal_start,
              std::unique_ptr<CFGNode>&& range,
              std::unique_ptr<CFGNode>&& int_literal_end,
              std::unique_ptr<CFGNode>&& loopStep) : CFGNode(NodeKind::LOOP_RANGE) {
...
class ForLoop : public CFGNode {
   public:
    ForLoop() : CFGNode(NodeKind::ERROR) {}
    ...
    ForLoop(std::unique_ptr<CFGNode>&& kw_for,
            ...
            std::unique_ptr<CFGNode>&& lbrace,
            std::unique_ptr<CFGNode>&& stmt_list,
            std::unique_ptr<CFGNode>&& rbrace) : CFGNode(NodeKind::FOR_LOOP) {
...
class ElseBlock : public CFGNode {
   public:
    ElseBlock(std::unique_ptr<CFGNode>&& kw_else,
              std::unique_ptr<CFGNode>&& lbrace,
              std::unique_ptr<CFGNode>&& stmt_list,
              std::unique_ptr<CFGNode>&& rbrace) : CFGNode(NodeKind::ELSE_BLOCK) {
    ...
    ElseBlock() : CFGNode{NodeKind::ELSE_BLOCK} {}
};

class ElifBlock : public CFGNode {
   public:
    ElifBlock() : CFGNode(NodeKind::ERROR) {}

    ElifBlock(std::unique_ptr<CFGNode>&& kw_elif,
              std::unique_ptr<CFGNode>&& expression,
              std::unique_ptr<CFGNode>&& lbrace,
              std::unique_ptr<CFGNode>&& stmt_list,
              std::unique_ptr<CFGNode>&& rbrace) : CFGNode(NodeKind::ELIF_BLOCK) {
...
class ElifBlocks : public CFGNode {
   public:
    ElifBlocks(std::unique_ptr<CFGNode>&& elif_block, std::unique_ptr<CFGNode>&& elif_blocks) : CFGNode{NodeKind::ELIF_BLOCKS} {
        addChildNode(std::move(elif_block));
        addChildNode(std::move(elif_blocks));
    }

    ElifBlocks() : CFGNode{NodeKind::ELIF_BLOCKS} {}
};

class IfBlock : public CFGNode {
   public:
    IfBlock() : CFGNode(NodeKind::ERROR) {}

    IfBlock(std::unique_ptr<CFGNode>&& kw_if,
            std::unique_ptr<CFGNode>&& expression,
            std::unique_ptr<CFGNode>&& lbrace,
            std::unique_ptr<CFGNode>&& stmt_list,
            std::unique_ptr<CFGNode>&& rbrace) : CFGNode(NodeKind::IF_BLOCK) {
...
class ReturnStmt : public CFGNode {
   public:
    ReturnStmt() : CFGNode(NodeKind::ERROR) {}

    ReturnStmt(std::unique_ptr<CFGNode>&& kw_return, std::unique_ptr<CFGNode>&& returnVar) : CFGNode{NodeKind::RETURN_STMT} {
...
class LoopStmt : public CFGNode {
   public:
    LoopStmt() : CFGNode(NodeKind::ERROR) {}

    LoopStmt(std::unique_ptr<CFGNode>&& loopType) : CFGNode{NodeKind::LOOP_STMT} {
...
class ConditionalStmt : public CFGNode {
   public:
    ConditionalStmt() : CFGNode(NodeKind::ERROR) {}
...
class AssignmentStmt : public CFGNode {
   public:
    AssignmentStmt() : CFGNode(NodeKind::ERROR) {}

    AssignmentStmt(std::unique_ptr<CFGNode>&& identifier, std::unique_ptr<CFGNode>&& assign, std::unique_ptr<CFGNode>&& expression) : CFGNode{NodeKind::ASSIGNMENT_STMT} {
        addChildNode(std::move(identifier));
...
class Stmt : public CFGNode {
   public:
    Stmt() : CFGNode(NodeKind::ERROR) {}

    Stmt(std::unique_ptr<CFGNode>&& stmtNode) : CFGNode(NodeKind::STMT) {
        addChildNode(std::move(stmtNode));
    }

    Stmt(std::unique_ptr<CFGNode>&& lbrace, std::unique_ptr<CFGNode>&& stmtNode, std::unique_ptr<CFGNode>&& rbrace) : CFGNode(NodeKind::STMT) {
        addChildNode(std::move(lbrace));
        addChildNode(std::move(stmtNode));
        addChildNode(std::move(rbrace));
    }
...
class StmtList : public CFGNode {
   public:
    StmtList(std::unique_ptr<CFGNode>&& stmt,
             std::unique_ptr<CFGNode>&& semi_colon,
             std::unique_ptr<CFGNode>&& stmtList) : CFGNode(NodeKind::STMT_LIST) {
        addChildNode(std::move(stmt));
        addChildNode(std::move(semi_colon));
        addChildNode(std::move(stmtList));
    }

    StmtList() : CFGNode(NodeKind::STMT_LIST) {}
};
